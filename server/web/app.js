const state = { token: sessionStorage.getItem('learning_access_token') || '', refreshToken: sessionStorage.getItem('learning_refresh_token') || '', refreshing: null, user: null, courses: [], questions: [], taskKnowledgePoints: [], practice: null, agentSessionId: Number(localStorage.getItem('learning_agent_session_id')) || null, agentSessions: [], tutorMode: 'Socratic' };
...
function showApp() { $('#auth-view').classList.add('hidden'); $('#app-view').classList.remove('hidden'); $('#user-label').textContent = state.user.display_name || state.user.email; $('#org-label').textContent = `${state.user.tenant_id.slice(0, 8)} · ${state.user.role}`; loadCurrentView('today'); }
...
async function openCourse(courseId) { try { const data = await api(`/courses/${courseId}/workspace`); const course = data.course; $('#course-list').innerHTML = `<section class="workspace-card"><button class="text-button" id="back-courses">← 返回课程</button><div class="course-hero"><div><p class="eyebrow">LEARNING WORKSPACE</p><h3>${escapeHtml(course.name)}</h3><p>${escapeHtml(course.description || course.subject)}</p></div><div class="course-progress"><strong>${course.progress}%</strong><span>课程进度</span></div></div><div class="workspace-tabs"><button class="primary" data-view="goals">查看计划与知识</button><button class="secondary" data-view="practice">开始练习</button><button class="secondary" data-view="mistakes">查看错题</button></div><div class="content-grid"><section><h4>知识掌握度</h4><div class="list">${data.knowledge.map(item => `<article class="list-row"><span>${escapeHtml(item.name)}</span><strong>${item.mastery}%</strong></article>`).join('') || '<span class="task-meta">还没有知识点。可在资料页或目标页建立知识结构。</span>'}</div></section><section><h4>目标与近期任务</h4><div class="list">${data.goals.map(item => `<article class="list-row"><div><strong>${escapeHtml(item.title)}</strong><div class="task-meta">${item.target_date} · ${item.progress}%</div></div></article>`).join('') || '<span class="task-meta">还没有课程目标。</span>'}</div></section></div></section>`; $('#back-courses').onclick = loadCourses; bindViewButtons(); } catch (error) { flash(error.message); } }
...
watchJob = async (...args) => { const root = $('#rag-job'); markJobState(root, 'processing'); try { const result = await productWatchJob(...args); const failed = /failed|失败|error|错误/i.test(root?.textContent || ''); markJobState(root, failed ? 'failed' : 'completed'); return result; } catch (error) { markJobState(root, 'failed'); throw error; } };
...
function workspaceTaskTodo(task) {
  ...
  return `<article class="course-task-todo ${completed ? 'is-done' : ''}"><label class="course-task-check"><input type="checkbox" data-workspace-task="${task.id}" ${completed ? 'checked disabled' : ''} aria-label="完成任务：${escapeHtml(task.title)}"><span></span></label><div class="course-task-copy"><strong>${escapeHtml(task.title)}</strong><div class="task-meta">${learningDate(task.planned_date)} · ${Number(task.duration_minutes || 0)} 分钟${task.scheduled_time ? ` · ${escapeHtml(task.scheduled_time)}` : ''}</div>${details ? `<div class="course-task-detail">${escapeHtml(details)}</div>` : ''}</div><span class="course-task-state">${completed ? '已完成' : (task.status === 'in_progress' ? '进行中' : '待完成')}</span></article>`;
}
