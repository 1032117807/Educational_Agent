...
function renderMermaidBlock(source, id) { const lines = String(source || '').split(/\r?\n/).filter(line => !/^\s*(graph|flowchart|gantt|sequenceDiagram)\b/i.test(line) && line.includes('-->')).slice(0, 24); const nodes = []; const edges = []; const add = raw => { const clean = raw.trim().replace(/^\|[^|]*\|\s*/, ''); const match = clean.match(/^([\w-]+)(?:\[([^\]]+)\]|\(([^)]+)\)|\{([^}]+)\})?/); if (!match) return null; const key = match[1]; if (!nodes.some(item => item[0] === key)) nodes.push([key, (match[2] || match[3] || match[4] || key).slice(0, 48)]); return key; }; lines.forEach(line => { const parts = line.split(/-->|---/); if (parts.length >= 2) { const from = add(parts[0]); const to = add(parts.slice(1).join(' ')); if (from && to) edges.push([from, to]); } }); const width = 760; const height = Math.max(150, Math.ceil(Math.max(nodes.length, 1) / 2) * 82 + 40); const positions = new Map(nodes.map((item, i) => [item[0], { x: 210 + (i % 2) * 340, y: 45 + Math.floor(i / 2) * 82 }])); const esc = value => String(value).replace(/[&<>"']/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&apos;' }[char])); const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}"><defs><marker id="arrow-${id}" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="#4386a4"/></marker></defs><rect width="100%" height="100%" fill="white"/>${edges.map(([a,b]) => { const p = positions.get(a), q = positions.get(b); return `<line x1="${p.x}" y1="${p.y+20}" x2="${q.x}" y2="${q.y-20}" stroke="#4386a4" stroke-width="2" marker-end="url(#arrow-${id})"/>`; }).join('')}${nodes.map(([key,label]) => { const p = positions.get(key); return `<g><rect x="${p.x-145}" y="${p.y-20}" width="290" height="40" rx="5" fill="#edf7fb" stroke="#4386a4"/><text x="${p.x}" y="${p.y+5}" text-anchor="middle" font-family="sans-serif" font-size="14" fill="#18394a">${esc(label)}</text></g>`; }).join('')}</svg>`; const svgUrl = URL.createObjectURL(new Blob([svg], { type: 'image/svg+xml' })); return `<section class="chat-mermaid"><img src="${svgUrl}" alt="AI 生成的流程图"><div class="chat-mermaid-actions"><a class="secondary" download="diagram.svg" href="${svgUrl}">下载 SVG</a><a class="secondary" href="#" data-png-svg="${escapeHtml(svg)}">下载 PNG</a></div></section>`; }
function renderMarkdownWithMermaid(content, messageId = '') { const source = String(content || ''); const pattern = /```mermaid\s*\n([\s\S]*?)```/gi; let html = ''; let last = 0; let match; let index = 0; while ((match = pattern.exec(source))) { html += `<div class="rich-markdown">${escapeHtml(source.slice(last, match.index)).replace(/\n/g, '<br>')}</div>`; html += renderMermaidBlock(match[1], `${messageId}-${index++}`); last = match.index + match[0].length; } html += `<div class="rich-markdown">${escapeHtml(source.slice(last)).replace(/\n/g, '<br>')}</div>`; return html; }
function renderRichBlocks(blocks, messageId = '') { return (blocks || []).map((block, index) => {
  if (block.type !== 'quiz') return renderMarkdownWithMermaid(block.content || '', `${messageId}-${index}`);
...
function bindMermaidDownloads(root) { root.querySelectorAll('[data-png-svg]').forEach(link => { if (link.dataset.bound) return; link.dataset.bound = '1'; link.onclick = event => { event.preventDefault(); const image = new Image(); image.onload = () => { const canvas = document.createElement('canvas'); canvas.width = 1520; canvas.height = 500; const context = canvas.getContext('2d'); context.fillStyle = '#fff'; context.fillRect(0, 0, canvas.width, canvas.height); context.drawImage(image, 0, 0, canvas.width, canvas.height); const download = document.createElement('a'); download.download = 'diagram.png'; download.href = canvas.toDataURL('image/png'); download.click(); }; image.src = URL.createObjectURL(new Blob([link.dataset.pngSvg], { type: 'image/svg+xml' })); }; }); }
function renderAgentMessages(messages) { const root = $('#agent-messages'); root.innerHTML = messages.map(message => `<article class="agent-message ${escapeHtml(message.role)}" data-agent-message-id="${message.id}"><strong>${message.role === 'user' ? 'You' : 'Learning Agent'}</strong><div>${renderAgentMessageContent(message)}</div></article>`).join('') || '<p class="task-meta">Start a conversation with your learning agent.</p>'; window.restoreSourceResults?.(root, messages); window.restoreAgentSourceResults?.(root, messages); bindAgentDownloads(root); bindMermaidDownloads(root); bindRichQuizzes(root); root.scrollTop = root.scrollHeight; }
...
$('#member-form').onsubmit = async event => { event.preventDefault(); try { await api('/organization/users', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); event.target.reset(); event.target.classList.add('hidden'); flash('Account created'); loadMembers(); } catch (error) { flash(error.message); } };
