...
fetch('/health/config').then(response => response.ok ? response.json() : null).then(config => { if (config && config.public_registration_enabled === false) { document.querySelector('[data-auth-tab="register"]')?.remove(); $('#register-form')?.remove(); } }).catch(() => {});
