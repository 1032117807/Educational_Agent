...
def test_pgvector_schema_accepts_384_dimensions() -> None:
    assert AISettings(embedding_dimensions=384).embedding_dimensions == 384


def test_local_embedding_does_not_require_an_api_key() -> None:
    settings = AISettings(enabled=False, api_key="", embedding_dimensions=384)
...
def test_enabled_chat_requires_server_side_api_key() -> None:
    with pytest.raises(ValueError, match="LEARNING_AI_API_KEY"):
        AISettings(enabled=True, api_key="", embedding_dimensions=384)
